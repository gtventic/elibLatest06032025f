...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_POST['delete_pdf'])) {
        ...
        if ($pdf_record) {
            ...
            $stmt->bindParam(':pdf_id', $pdf_id, PDO::PARAM_INT);
            if ($stmt->execute()) {
                // Get the public IP address of the user deleting the file.
                $ip_address = $_SERVER['REMOTE_ADDR'];
                // Log the deletion action—including the PDF file name—into pdf_log.
                $log_stmt = $conn->prepare("INSERT INTO pdf_log (user_id, pdf_file_id, pdf_file_name, view_time, action, ip_address) VALUES (:user_id, :pdf_file_id, :pdf_file_name, NOW(), 'delete', :ip_address)");
                $log_stmt->bindParam(':user_id', $_SESSION['user']['id'], PDO::PARAM_INT);
                $log_stmt->bindParam(':pdf_file_id', $pdf_id, PDO::PARAM_INT);
                $log_stmt->bindParam(':pdf_file_name', $pdf_record['file_name']);
...
// Retrieve system users (for example, those with role 'user').
  ...
  <div class="container mt-4">
    ...
    <form method="POST" enctype="multipart/form-data">
      ...
      <div class="mb-3">
        ...
        <small class="form-text text-muted">Hold the Ctrl (Windows) or Command (Mac) key to select multiple users.</small>
      </div>
      ...
      <tbody>
        <?php if (!empty($pdf_files)) {
          foreach ($pdf_files as $pdf) {
              // Process allowed users column into names.
              $allowedNames = "";
