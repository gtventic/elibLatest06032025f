...
// It uses COALESCE to display the file name from pdf_files if available,
// otherwise falling back to pdf_log.pdf_file_name.
$sql = "SELECT 
            pl.id, 
            u.username, 
            COALESCE(pf.file_name, pl.pdf_file_name) AS file_name, 
            pl.view_time, 
            pl.action, 
            pl.ip_address
        FROM pdf_log pl 
...
<body>
  ...
  <script>
      ...
      function timerIncrement() {
          idleTime++;
          if (idleTime > 60) { // 60 seconds idle time
