        ...
        <?php if (count($pdfs) > 0): ?>
          <?php foreach ($pdfs as $pdf): ?>
            <tr>
              ...
              <td>
                <?php
                  // Determine the folder based on the category field
                  $folder = "";
                  ...
                  // Build the file URL using the folder (if defined) and the file name.
                  // If folder is empty, fall back to the base uploads folder.
                  $fileURL = "../uploads/" . ($folder ? $folder . "/" : "") . htmlspecialchars($pdf['file_name']);
                ?>
                <a class="btn btn-primary btn-sm" target="_blank" href="<?php echo $fileURL; ?>">View</a>
              </td>
            </tr>
          <?php endforeach; ?>
        <?php else: ?>
